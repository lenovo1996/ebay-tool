<?php
// $Id: EbatNs_ResponseParser.php,v 1.2 2008-06-05 08:21:06 michael Exp $
...
class EbatNsReturnManagement_ResponseParser
{
    public $_client;
     ...
     *
     * @param EbatNsReturnManagement_SimpleType $element
    ...
    function _includeType ($typeName)
    {
        if (! class_exists($typeName))
        {
            ...
            if ($typeFileName == 'getUserReturnsResponse')
            {
                require_once 'getUserReturnsResponse.php';
            }
            else if ($typeFileName == 'getReturnDetailResponse')
            {
                require_once 'getReturnDetailResponse.php';
            }
            else
            {
                require_once ucfirst($typeFileName) . '.php';    
            }
        }
    ...
    {
    	$messageText = str_replace('ns1:', '', $messageText); 
        ...
        if ($responseTypeName != null)
            ...
            $this->_responseTypeName = $responseName;
        ...
        if (! xml_parse($parser, $messageText, true))
        {
            ...
            $errResponse = new EbatNsReturnManagement_ResponseError();
        ...
        if ($this->_responseObject === null && $responseName != 'errorMessage')
        {
            if (($errorMessageResponseObject = $this->decode('errorMessage', $messageText, $parseMode, $responseTypeName)) && $errorMessageResponseObject instanceof ErrorMessage)
        	{
                if ($responseName == 'getUserReturnsResponse')
                {
                   require_once 'getUserReturnsResponse.php';
                }
                else if ($responseName == 'getReturnDetailResponse')
                {
                    require_once 'getReturnDetailResponse.php';
                }
                else
                {
                    require_once ucfirst($responseName) . '.php';    
                }
        		
        		if (class_exists($responseName))
        		{
        			$responseObject = new $responseName();
        			$responseObject->setack(AckValue::CodeType_Failure);
        			$responseObject->setversion(EBAY_WSDL_VERSION);
        			$responseObject->settimestamp($this->_decodeData(gmdate('Y-m-d\TH:i:s.000Z'), 'dateTime'));
        			$responseObject->seterrorMessage($errorMessageResponseObject);
        			$this->_reduceElement($responseObject);
